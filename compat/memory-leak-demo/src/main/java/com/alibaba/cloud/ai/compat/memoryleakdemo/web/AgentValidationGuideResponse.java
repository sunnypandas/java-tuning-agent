	...
	public static AgentValidationGuideResponse buildDefault() {
		return new AgentValidationGuideResponse(
				...
				List.of(
						...
						new ValidationScenarioView("runnable-high-cpu-thread",
								"RUNNABLE high CPU thread",
								"Starts background CPU workers with stable thread names and a clear hot method so Thread.print CPU rows and JFR execution samples can point back to source.",
								List.of("POST /api/leak/cpu/start before collecting evidence",
										"collectMemoryGcEvidence with includeThreadDump=true + confirmationToken while workers are running",
										"Optionally record a profile JFR during the same window",
										"Stop with POST /api/leak/cpu/stop after evidence collection"),
								List.of("Thread dump shows runnable CPU hotspot", "JFR shows hottest execution samples",
										"CpuBurnService.burnCpuLoop"),
								"-Xms128m -Xmx256m -XX:+UseG1GC",
								List.of(
				...
				List.of("SafeJvmRuntimeCollector expects G1 GC.heap_info shape; use -XX:+UseG1GC.",
						...
						"For high CPU validation, keep /api/leak/cpu/start running while collecting Thread.print or JFR evidence, then call /api/leak/cpu/stop.",
